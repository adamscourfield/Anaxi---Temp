...
- **Observation Details Enhancement**: Fixed display of complete observation information across the platform:
  - **Class Field Fix**: Corrected field name mismatch (`classGroup` → `classInfo`) to display class information in observation tables on dashboard and observation history pages. Class column now properly shows values like "Year 9C", "Year 10A", etc.
  - **Details Page Completion**: Enhanced `FeedbackReport` component to display all observation details including lesson topic, class, and qualitative feedback. Details page now shows comprehensive observation information with icons and proper formatting.
  - **Type Safety**: Fixed TypeScript type mismatches in observation-history page mapping to ensure `classInfo` is properly typed as `string | undefined`.
- **Comprehensive Analytics Dashboard Overhaul**: Major expansion of the analytics dashboard with four new analytics capabilities:
  1. **Lowest Performing Teachers**: Displays teachers who need support with time period filtering (week/month/year/all-time), helping administrators identify teachers who may benefit from mentorship or additional training.
  2. **Staff-Filtered Observation Trends**: Multi-select staff filter allows viewing observation activity for specific teachers or observers, enabling targeted analysis of individual staff observation patterns.
  3. **Quality & Quantity Trend Graph**: Dual-axis line graph showing both observation count (bar chart) and average quality scores (line chart) over time with week/month/year views, providing insight into whether observation quality improves over time.
  4. **Qualitative Feedback Summary**: Displays the 10 most recent qualitative feedback entries from observations school-wide, surfacing common themes and actionable insights from observer comments.
- **Category Performance Time Filtering**: Added week/month/year time period filter to Category Performance section on dashboard. Users can now select a time window (last week, last month, or last year) to view category-specific performance metrics for that period. The filter uses rolling date windows and automatically updates the analytics display when changed.
- **Dashboard Analytics Fix**: Fixed critical bugs in dashboard analytics section. Teacher names now display correctly instead of showing "Unknown" by fetching and mapping user data from school memberships. Category performance data now displays correctly by bulk-loading observation habits and categories, calculating scores based on the ObservationHabit schema (observed=1, not observed=0). Optimized endpoint performance from O(N×M) sequential queries to parallel batch queries (N observation habits + K unique categories), reducing query count from thousands to hundreds for large datasets.
- **Meeting Creator Auto-Add**: Meeting creators are now automatically added as attendees when creating a meeting. The creator is added with attendeeRole="Organizer" and attendanceStatus="attended". To reduce clutter, the creator is automatically filtered out from the attendees list in both the meetings table and meeting details page (they don't see themselves in the list). Backend enriches attendee data with userId field to enable this filtering. This provides a cleaner UX where users don't have to manually add themselves or see their own name in attendee lists.
- **Leave Requests UI Polish**: Streamlined action buttons on approve-leave page to icon-only buttons with tooltips, reducing horizontal space by ~70% while maintaining clarity. Buttons now include: Eye (view details), Dollar sign (approve with pay), Check (approve without pay), and X (deny).
- **Leave Requests Overhaul**: Complete redesign of both leave request pages with professional table layouts. The "Request Leave" page (`/leave-requests`) displays user's own requests in a table format with columns for Teacher, Type, Dates, Status, and Submitted date, plus a collapsible form for submitting new requests with medical documentation upload. The "Approve Leave" page (`/approve-leave`) uses the same table design for reviewing pending requests, with search functionality, status filter tabs (All, Pending, Approved with/without pay, Denied), and action buttons for approval workflow. Both pages include detailed view dialogs showing complete request information. Backend enriches leave request data with requester and approver names from user tables. Medical documentation uploads use private object storage with ACL security fix to honor visibility settings (defaults to private).
- **Login Error Handling Fix**: Fixed invalid credentials being shown as runtime errors instead of form validation errors. The login form now uses `mutate()` instead of `mutateAsync()` to prevent unhandled promise rejections. Invalid credentials now display as a clear error message on the form, allowing users to correct their input and try again.
- **Password Reset Session Fix**: Fixed blank screen issue after password reset/setup. The backend now properly destroys sessions and clears session cookies when passwords are changed, ensuring users must log in with their new credentials. Frontend redirects to `/login` instead of `/` after password changes, preventing stuck or blank screens. Dashboard loads properly after successful login with all data intact.
...
- **Security Enhancements**: Implemented global `sanitizeUser()` function across all user-related API endpoints to prevent sensitive data leakage. Fixed object storage ACL endpoint to respect requested visibility settings instead of hard-coding to public.
