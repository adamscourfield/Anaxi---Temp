...
- **Password Reset Redirect Fix**: Fixed blank screen issue after password reset/setup. Both reset-password and set-password pages now correctly redirect to `/login` instead of `/` after successful password changes, ensuring users can log in with their new credentials without getting stuck.
