...
- **Rubric Persistence**: Fully implemented full-stack persistence for rubrics, categories, and habits. Categories and habits now persist correctly across navigation using PostgreSQL database storage. Backend automatically calculates order fields for proper display sequencing. Frontend uses React Query for efficient caching and real-time updates.
- **Observation Permissions Model**: Clarified that users can **conduct observations** on anyone in their school (no permissions required), but can only **view observations** of teachers they have explicit permission for (plus their own).
- **Security Enhancements**: Implemented global `sanitizeUser()` function across all user-related API endpoints to prevent sensitive data leakage (password hashes, setup tokens, reset tokens).
